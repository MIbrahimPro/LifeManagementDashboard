...
export const CardChecklist: FC<CardChecklistProps> = ({ categoryId, isDarkMode }) => {
    ...
    const [addToDietName, setAddToDietName] = useState('');
    const [addToExerciseName, setAddToExerciseName] = useState('');
    ...
    const handleAddToDiet = async () => {
        if (!addToDietName.trim()) return;
        await addCardSection(categoryId, addToDietName.trim(), 'custom', true, 'diet');
        setAddToDietName('');
    };

    const handleAddToExercise = async () => {
        if (!addToExerciseName.trim()) return;
        await addCardSection(categoryId, addToExerciseName.trim(), 'custom', true, 'exercise');
        setAddToExerciseName('');
    };
    ...
    const goalsSections = otherSectionsRaw.filter((s: { kind: string }) => s.kind === 'goals');
    const otherSections = otherSectionsRaw.filter((s: { kind: string }) => s.kind !== 'goals');

    return (
        <div className="space-y-4">
            {categoryId === 'family' && (
                <FamilyFriendsGrid isDarkMode={isDarkMode} />
            )}
            {categoryId === 'income' && (
                <IncomeExpensesView isDarkMode={isDarkMode} />
            )}
            {categoryId === 'assets' && (
                <AssetsLiabilitiesView isDarkMode={isDarkMode} />
            )}
            {categoryId === 'physical' && (
                <>
                    ...
                    {dietSections.length >= 0 && (
                        <div className="mb-3">
                            <h3 style={{ color: isDarkMode ? '#f3f4f6' : '#111827', borderColor: isDarkMode ? '#4b5563' : '#e5e7eb' }} className="text-lg font-bold mb-2 border-b pb-1">
                                Diet
                            </h3>
                            <div className="space-y-3">
                                ...
                                <div className="flex gap-2 items-center">
                                    <input
                                        value={addToDietName}
                                        onChange={(e) => setAddToDietName(e.target.value)}
                                        onKeyDown={(e) => e.key === 'Enter' && handleAddToDiet()}
                                        placeholder="Add category..."
                                        style={inputStyle}
                                        className="w-28 max-w-[120px] shrink-0 px-2 py-1.5 text-sm border rounded"
                                    />
                                    <button
                                        onClick={handleAddToDiet}
                                        disabled={!addToDietName.trim()}
                                        className="shrink-0 flex items-center gap-1 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white px-3 py-1.5 rounded text-sm font-medium"
                                    >
                                        <Plus size={14} /> Add to Diet
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    {exerciseSections.length >= 0 && (
                        <div className="mb-3">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex gap-2 items-center">
                                    <input
                                        value={addToExerciseName}
                                        onChange={(e) => setAddToExerciseName(e.target.value)}
                                        onKeyDown={(e) => e.key === 'Enter' && handleAddToExercise()}
                                        placeholder="Add category..."
                                        style={inputStyle}
                                        className="w-28 max-w-[120px] shrink-0 px-2 py-1.5 text-sm border rounded"
                                    />
                                    <button
                                        ...
                                        disabled={!addToExerciseName.trim()}
                                        className="shrink-0 flex items-center gap-1 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white px-3 py-1.5 rounded text-sm font-medium"
                                    >
                                        <Plus size={14} /> Add to Exercise
                                    </button>
                                </div>
                            </div>
                        </div>
...
function TextEntryRow({
    ...
    return (
        <div className="flex items-center gap-2">
            <label style={{ color: isDarkMode ? '#d1d5db' : '#374151' }} className="text-sm shrink-0 min-w-20">{entryName}</label>
            <input
                type="text"
                value={localValue}
            ...
            />
            <button onClick={onDelete} className="p-0.5 text-red-500 opacity-70 hover:opacity-100"><Trash2 size={12} /></button>
        </div>
    );
}
                ...
                {(entries || []).map((entry) => {
                    ...
                    return (
                        <TextEntryRow
                            ...
                            value={value}
                            onBlur={(v) => toggle(entry.id, completed, v)}
                            onDelete={() => deleteSectionEntry(entry.id)}
